...
// CHANGED TO ACCUMULATION ALGORITHM IN BOTH DIRECTION
			...
			for(int senterY = 0; senterY < imageIn->x; senterY++) {
				...
				if (senterY-(size+1)<0){
					...
					for(int y = -size; y <= size; y++) {
						...
						if(senterY >= imageIn->y)
							continue;
						
					int offsetOfThePixel = (numberOfValuesInEachRow * (senterY-(size+1)) + senterX);
					sum1 = sum1 - imageIn->data[offsetOfThePixel].red;
					sum2 = sum2 - imageIn->data[offsetOfThePixel].green;
					sum3 = sum3 - imageIn->data[offsetOfThePixel].blue;
					countIncluded--;
				}
				else{
				
					int offsetOfThePixel_remove = (numberOfValuesInEachRow * (senterY-(size+1)) + senterX);
					int offsetOfThePixel_add = (numberOfValuesInEachRow * (senterY+(size)) + senterX);
					sum1 = sum1 - imageIn->data[offsetOfThePixel_remove].red + imageIn->data[offsetOfThePixel_add].red ;
					sum2 = sum2 - imageIn->data[offsetOfThePixel_remove].green + imageIn->data[offsetOfThePixel_add].green ;
					sum3 = sum3 - imageIn->data[offsetOfThePixel_remove].blue + imageIn->data[offsetOfThePixel_add].blue ;
				}
	...
	for(int senterY = 0; senterY < temp->y; senterY++) {
		...
		for(int senterX = 0; senterX < temp->x; senterX++) {
			...
			if (senterX-(size+1)<0){
				...
				int offsetOfThePixel1_add = (numberOfValuesInEachRow * senterY + senterX+(size));
				sum1 = sum1 - temp->data[offsetOfThePixel_remove].red + temp->data[offsetOfThePixel1_add].red ;
				sum2 = sum2 - temp->data[offsetOfThePixel_remove].green + temp->data[offsetOfThePixel1_add].green ;
				sum3 = sum3 - temp->data[offsetOfThePixel_remove].blue + temp->data[offsetOfThePixel1_add].blue ;
			}
