...
void blurIteration(AccurateImage *imageOut, AccurateImage *imageIn, AccurateImage *temp, int size) {
	...
	v4sd *temp_1;
	//temp_1 = malloc(sizeof(AccurateImage));
	temp_1 = malloc(imageIn->x * imageIn->y * sizeof(AccuratePixel));
	...
	for(int senterY = 0; senterY < imageIn->y; senterY++) {
		for(int senterX = 0; senterX < imageIn->x; senterX++) {
			...
			for(int y = -size; y <= size; y++) {
				...
				v4sd temp_temp = {imageIn->data[offsetOfThePixel].red, imageIn->data[offsetOfThePixel].green, imageIn->data[offsetOfThePixel].blue, 0};
				sum = sum + temp_temp;
				//temp->data[offsetOfThePixel] = vbector;
	...
	for(int senterY = 0; senterY < temp->y; senterY++) {
		...
		for(int senterX = 0; senterX < temp->x; senterX++) {
			...
			for(int x = -size; x <= size; x++) {
				...
				sum = sum + temp_1[offsetOfThePixel];
			...
			int numberOfValuesInEachRow = imageOut->x; // R, G and B
			int offsetOfThePixel = (numberOfValuesInEachRow * senterY + senterX);
			imageOut->data[offsetOfThePixel].red = value[0];
			imageOut->data[offsetOfThePixel].green = value[1];
			imageOut->data[offsetOfThePixel].blue = value[2];
		}
...
int main(int argc, char** argv) {
	...
	#pragma omp parallel sections shared(imageAccurate1_tiny, imageAccurate2_tiny, imageAccurate1_small, imageAccurate2_small, imageAccurate1_medium, imageAccurate2_medium, imageAccurate1_large, imageAccurate2_large)
	{ 
		#pragma omp section
		{
				AccurateImage *temp = convertToAccurateImage(image);
